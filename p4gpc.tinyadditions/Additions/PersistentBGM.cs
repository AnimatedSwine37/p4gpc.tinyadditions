...
namespace p4gpc.tinyadditions.Additions
{
    class PersistentBGM : Addition
    {
        ...
        public PersistentBGM(Utils utils, int baseAddress, Config configuration, IMemory memory, IReloadedHooks hooks) : base(utils, baseAddress, configuration, memory, hooks)
        {
            ...
            _utils.SigScan("E8 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 8B 81 ?? ?? ?? ?? 8B 40 ?? 83 08 01", "results start bgm", InitResultsStartBgmHook);
        ...
        private void InitResultsStartBgmHook(int address)
        {
            ...
            _resultsStartBgmHook = _hooks.CreateAsmHook(function, address - 2, AsmHookBehaviour.ExecuteFirst).Activate();
